...
<!-- VitTrade install note: this skill is REFERENCE/INSPIRATION intelligence
     only. Repo contracts always win in a conflict: AGENTS.md, DESIGN.md and
     docs/02_FLUTTER_MIGRATION/standards/ own the tokens (colors, radii,
     typography, spacing), the Vit* component ladder, and the guardrails.
     Never let this skill's generated palettes/fonts/styles introduce new
     hex values, font sizes, or components into flutter_app/** — use its
     searches to critique/justify composition, density, hierarchy, and UX
     patterns, then map any accepted idea onto existing tokens. -->
