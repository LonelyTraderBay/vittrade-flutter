...
| [Tablet composition tier](./standards/Tablet-Composition-Tier-Standard.md) | Every tablet page library builds a sanctioned registry scaffold (dashboard / master shell / pane scaffold / section frame / page layout / feature surface); T1 ratchet baseline + T2 absolute (zero `VitAutoHidePageScaffold(`, zero hand-rolled `class …Frame`) | `tool/tablet_composition_tier_audit.dart` | — (baseline equality enforced by the tool; C1–C3 stay in `tablet_composition_guardrail_test.dart`) | `dart run tool/tablet_composition_tier_audit.dart --check` | Named CI step + preflight (parses CI) |
