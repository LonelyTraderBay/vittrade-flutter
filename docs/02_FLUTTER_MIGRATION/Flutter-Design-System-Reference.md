...
| [Tablet spacing & gutters](./standards/Tablet-Spacing-Gutter-Standard.md) | Closed Base-8-derived role→token map + frame geometry (`TabletDashboardWidths`); zero literal spacing / element-level separator SizedBox in tablet files (S1–S7), with explicit module/shared surface-aware mappings | `tool/tablet_spacing_audit.dart` | `tablet_spacing_guardrail_test.dart` + `tablet_base8_role_scale_guardrail_test.dart` + `tablet_module_role_scale_guardrail_test.dart` + `tablet_gap_12_guardrail_test.dart` + `tablet_rhythm_role_guardrail_test.dart` | `dart run tool/tablet_spacing_audit.dart --check` | Guardrails CI job (whole `test/quality`) + full suite |
