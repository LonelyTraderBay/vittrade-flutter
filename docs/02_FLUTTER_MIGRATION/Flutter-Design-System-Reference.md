...
| [Tablet spacing & gutters](./standards/Tablet-Spacing-Gutter-Standard.md) | Role→token spacing map + frame geometry (`TabletDashboardWidths`); zero literal spacing / element-level separator SizedBox in tablet files (S1–S4) | `tool/tablet_spacing_audit.dart` | `tablet_spacing_guardrail_test.dart` | `dart run tool/tablet_spacing_audit.dart --check` | Guardrails CI job (whole `test/quality`) + full suite |
