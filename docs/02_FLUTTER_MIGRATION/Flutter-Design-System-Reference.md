...
| [Tablet-adaptive layout](./standards/Tablet-Adaptive-Standard.md) | Dispatcher pattern, independent-scroll two-column dashboard, per-column width caps, phone reference untouched | — | Each tablet page's own widget test at its two-column width (e.g. `home_tablet_page_test.dart`) plus `test/shared/layout/vit_two_column_tablet_dashboard_test.dart` for the shared scaffold itself | `flutter test <touched tablet page test file> --reporter=compact` | **No dedicated tool for R1-R3/R9 — R4-R8 enforced by construction via the shared `VitTwoColumnTabletDashboard` widget (`lib/shared/layout/`); 5 reference screens (Home/Wallet/Markets/Trade/Profile)** |
