...
| --- | --- | --- |
| Spacing scale (Phone) | `AppSpacing.x1..x7` (3/5/8/13/21/34/55), `contentPad` (20), `rowPy` (14), `inputHeight`/`ctaHeight` (52) | `lib/app/theme/app_spacing.dart` |
| Tablet spacing role scale | `TabletSpacingTokens` closed Base-8-derived values (4/8/12/16/24/32/56), `contentPad` (20), `pageEndBreathing` (32) | `lib/app/theme/spacing/tablet_spacing_tokens.dart` |
...
| Page rhythm (layout) | `VitPageContent` wiring, structural nesting, vertical-rhythm tier per page | `tool/page_rhythm_audit.dart` | `page_rhythm_audit_sync_guardrail_test.dart` + `page_rhythm_spacing_scale_guardrail_test.dart` | `dart run tool/page_rhythm_audit.dart --check --strict-full` | Named CI step + artifact upload |
...
| [Phone composition](./standards/Phone-Composition-Standard.md) | Phone one-column grammar + module composition archetypes (job-picked row: feed hub / list+filters / terminal / settings hub / money flow); foundation machine-enforced by the audits below it | — (rides on Home reference + page rhythm + token audits) | `home_reference_consistency_guardrail_test.dart` + the structural guardrails | `flutter test test/quality/home_reference_consistency_guardrail_test.dart --reporter=compact` | Full-suite + named Home-reference step (composition layer is prose + Home diff, same split as tablet R2/R3/R9) |
| [Tablet spacing & gutters](./standards/Tablet-Spacing-Gutter-Standard.md) | Closed Base-8-derived role→token map + frame geometry (`TabletDashboardWidths`) + optical rhythm reference; zero literal spacing / element-level separator SizedBox in tablet files (S1–S7), S8 zero-gap two-line text stack (2026-09-14), gap-ROLE R1–R5 (2026-09-16, R6 thêm 2026-09-18), S6 gutter-flush gắn ngữ cảnh route 3 shell (2026-09-17), with explicit module/shared surface-aware mappings | `tool/tablet_spacing_audit.dart` + `tool/tablet_text_stack_gap_audit.dart` (S8) + `tool/tablet_gap_role_audit.dart` (R1–R6) + `tool/tablet_gutter_flush_audit.dart` (S6 route-context) | `tablet_spacing_guardrail_test.dart` + `tablet_base8_role_scale_guardrail_test.dart` + `tablet_module_role_scale_guardrail_test.dart` + `tablet_gap_12_guardrail_test.dart` + `tablet_gap_role_guardrail_test.dart` + `tablet_gutter_flush_guardrail_test.dart` + `tablet_rhythm_role_guardrail_test.dart` | `dart run tool/tablet_spacing_audit.dart --check` + `dart run tool/tablet_text_stack_gap_audit.dart --check` + `dart run tool/tablet_gap_role_audit.dart --check` + `dart run tool/tablet_gutter_flush_audit.dart --check` | Guardrails CI job (whole `test/quality`) + named CI steps (S8/R1–R6/S6-context) + full suite |
| [Tablet card & border](./standards/Tablet-Card-Border-Standard.md) | `VitCard` as the one frame source, role-based radius tiers, 3-step border tints (R1–R3 — baseline empty since 2026-08-23) | `tool/tablet_card_border_audit.dart` | `tablet_card_border_guardrail_test.dart` | `dart run tool/tablet_card_border_audit.dart --check` | Guardrails CI job (whole `test/quality`) + full suite |
| [Tablet-adaptive layout](./standards/Tablet-Adaptive-Standard.md) | Dispatcher pattern, independent-scroll two-column dashboard, per-column width caps, phone reference untouched, module composition archetypes, master-detail selection rules (route-derived), orientation policy (both orientations, zero orientation dispatch) | `tool/tablet_route_surface_audit.dart` (R1 runtime-dispatch half + R1c orientation-dispatch) | `tablet_route_surface_guardrail_test.dart` + `tablet_rotation_guardrail_test.dart` (rotation relayout) + each tablet page's own widget test at its two-column width (e.g. `home_tablet_page_test.dart`) plus `test/shared/layout/vit_two_column_tablet_dashboard_test.dart` for the shared scaffold itself | `dart run tool/tablet_route_surface_audit.dart --check` + `flutter test test/quality/tablet_rotation_guardrail_test.dart --reporter=compact` + `flutter test <touched tablet page test file> --reporter=compact` | R4-R8 enforced by construction via the shared `VitTwoColumnTabletDashboard` widget (`lib/shared/layout/`); 4 dashboard reference screens (Home/Wallet/Trade/Profile) + the Markets Analysis-terminal master-detail shell (`MarketsTabletMasterShell`, 2026-08-24); R2/R3/R9 + selection rules stay prose + widget-test |
| [Tablet composition tier](./standards/Tablet-Composition-Tier-Standard.md) | Every tablet page library builds a sanctioned registry scaffold (dashboard / pane workspace trong detail shell (2026-09-17) / master shell / pane scaffold / section frame / page layout / feature surface); T1 ratchet baseline + T2 absolute (zero `VitAutoHidePageScaffold(`, zero hand-rolled `class …Frame`) | `tool/tablet_composition_tier_audit.dart` | `test/shared/layout/vit_tablet_pane_workspace_test.dart` (scaffold pane-workspace riêng) + baseline equality enforced by the tool; C1–C3 stay in `tablet_composition_guardrail_test.dart` | `dart run tool/tablet_composition_tier_audit.dart --check` | Named CI step + preflight (parses CI) |
...
| [Motion](./standards/Motion-Standard.md) | Animation duration/easing tokens (`AppMotion`) + reduced-motion contract; tablet absolute-zero, phone ratchet baseline (`motion_phone_baseline.txt`) | `tool/motion_audit.dart` | `motion_guardrail_test.dart` | `dart run tool/motion_audit.dart --check` | Guardrails CI job (whole `test/quality`) + full suite |
| [Typography](./standards/Typography-Standard.md) | Role-based type scale (`app_text_styles.dart` ~42 roles); financial figures always tabular (`amount*`/`numeric*`) | — (locks the token file) | `typography_scale_guardrail_test.dart` + `design_token_consistency_audit` (fontSize literals) | `flutter test test/quality/typography_scale_guardrail_test.dart` | Guardrails CI job + full suite |
| [Data tables](./standards/Data-Table-Standard.md) | Financial table/list rules: tabular figures, token column widths, hairline separators, reachable sort, shared row primitives, masked sensitive cells | — (composed from locked rules) | — (review + the audits it composes) | See the standard's Verify block | Audit rides the composed locks; own tool at ≥2 columnar tables |
| [Contrast floor](./standards/Flutter-Native-Design-Standard.md) | WCAG 2.x ratios for core text/background token pairs (4.5:1 body text); 3 known deviations locked at current ratios (ratchet: improve-or-report) | — (locks the token pairs) | `contrast_floor_guardrail_test.dart` (math self-test + token-resolution guard) | `flutter test test/quality/contrast_floor_guardrail_test.dart --reporter=compact` | Guardrails CI job (whole `test/quality`) + full suite |
...
| [Bottom sheet usage](./standards/Bottom-Sheet-Standard.md) | `showVitBottomSheet` instead of raw `showModalBottomSheet`; tablet: pop-over cap 480dp auto + `VitSheetPanel`/tier/footer (audit) | `tool/tablet_sheet_audit.dart` | `bottom_sheet_guardrail_test.dart` + `vit_bottom_sheet_test.dart` | `dart run tool/tablet_sheet_audit.dart --check` + `flutter test test/quality/bottom_sheet_guardrail_test.dart` | Named CI step |
...
- [Phone-Composition-Standard.md](./standards/Phone-Composition-Standard.md) — Phone composition (one-column grammar + module archetypes by job)
...
- [Tablet-Composition-Tier-Standard.md](./standards/Tablet-Composition-Tier-Standard.md) — Tablet composition tier (registry scaffold per page library, T1 ratchet + T2 absolute)
...
- [Motion-Standard.md](./standards/Motion-Standard.md) — Motion (duration/easing tokens, reduced motion; tablet absolute + phone ratchet)
- [Typography-Standard.md](./standards/Typography-Standard.md) — Typography (role map, tabular figures)
- [Data-Table-Standard.md](./standards/Data-Table-Standard.md) — Financial data tables/lists
...
1. Pick the right scaffold: `VitPageLayout` + `VitPageContent` (never a raw `Scaffold`). Pick a `VitPageRhythm` tier — `.compact` (tab root/feed), `.standard` (scroll list), `.form` (wizard/KYC), `.relaxed` (hero/onboarding) — see `Page-Rhythm-Standard.md`. For a **phone** screen, first pick its composition archetype by the module's job (feed hub / list+filters / terminal / settings hub / money flow) — see [Phone-Composition-Standard.md](./standards/Phone-Composition-Standard.md).
...
3. Tokens only — no hardcoded `fontSize`/`EdgeInsets`/`BorderRadius.circular`. Use `AppSpacing.*` on Phone and `AppSurfaceSpacing.*`/`TabletSpacingTokens.*` on Tablet, with `AppRadii.*` and `AppColors.*` on both; new one-off tokens go in that feature's own `<module>_spacing_tokens.dart` only when the role is genuinely module-specific.
4. Interaction states and motion are tokens too: hover/focus ride on the shared widgets (`VitCard`/`VitCtaButton`/rows/`VitInput`) via `AppInputStates` — never a page-local `MouseRegion`/`onHover` or off-token `hoverColor:`/`focusColor:`; animation durations/easings are `AppMotion.*` — never an inline `Duration(`/`Curves.` in tablet files; see [Tablet-Input-Standard.md](./standards/Tablet-Input-Standard.md) and [Motion-Standard.md](./standards/Motion-Standard.md).
5. Match the pattern to the content: horizontal strip tile → `Card-Tile-Standard.md`; grid tile with a corner badge → `Service-Tile-Badge-Standard.md`; mission/task row → `Task-Card-Standard.md`; module row icon → `Accent-Icon-Box-Standard.md`; any tab/toggle/preset/filter → `Segment-Pill-Standard.md`; a page with 2+ tabbed content panels or a data-entry wizard → [Flutter-Page-Archetype-Standard.md](./standards/Flutter-Page-Archetype-Standard.md); a financial table/list (pairs, positions, ledger) → [Data-Table-Standard.md](./standards/Data-Table-Standard.md); every text style picked by role, figures tabular → [Typography-Standard.md](./standards/Typography-Standard.md).
6. Bottom sheets always go through `showVitBottomSheet` — never `showModalBottomSheet` directly; see [Bottom-Sheet-Standard.md](./standards/Bottom-Sheet-Standard.md). Post-action acknowledgements (success / error / coming-soon / must-ack) use `showVitNoticeSheet` — never `SnackBar` or `Positioned` success toasts; sticky footers are for form CTAs only — see [Notice-Acknowledgement-Standard.md](./standards/Notice-Acknowledgement-Standard.md).
7. Match one of the existing header archetypes and back-navigation contract — don't hand-roll a custom header or back button without a documented exception; see [Top-Header-Standard.md](./standards/Top-Header-Standard.md) and [Back-Navigation-Standard.md](./standards/Back-Navigation-Standard.md). Scroll-to-hide headers must go through `VitAutoHideHeaderScaffold` (collapse-budget gate) — never a page-local `heightFactor` hide; see [Scroll-Auto-Hide-Standard.md](./standards/Scroll-Auto-Hide-Standard.md).
8. High-risk flows (withdraw/escrow/security/address/payment-method) use `VitHighRiskStatePanel` with a `highRiskContractId`; see [High-Risk-State-Standard.md](./standards/High-Risk-State-Standard.md).
9. If the page is a dashboard/hub with 2+ independent content groupings, decide whether it needs a dedicated tablet layout (most screens don't — the shared tablet shell's nav rail is enough) — pick the composition by the module's actual job via the **module composition archetype table** in [Tablet-Adaptive-Standard.md](./standards/Tablet-Adaptive-Standard.md) (monitor dashboard / settings master-detail / money-movement detail / linear detail / wizard): modules differ by *what goes in the columns*, never by tokens or interaction rules.
10. Run before opening a PR (from `flutter_app/`):
   ```bash
   ...
   ```
11. If the new route is reachable via a nav call or link, confirm it shows up correctly in `dart run tool/navigation_edge_audit.dart` output — dangling/unlinked routes are a common miss. When a route's builder selects a dedicated Tablet/Web page, run `page_rhythm_screen_rollup.dart` and `page_rhythm_coverage_matrix.dart` explicitly (both listed in step 10, easy to skip since they're siblings of `page_rhythm_audit.dart` with separate `--check` state); do not add a feature-specific responsive dispatcher after P8.
...
13. For anything not covered above, check §2's domain map.
