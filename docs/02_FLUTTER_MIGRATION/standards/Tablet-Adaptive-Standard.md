...
1. **Full-width banner via the scaffold's `banner:` slot, morphology chosen by surface semantics** — *monitor* surfaces (Home/Markets/Trade) use a flat KPI strip: one `VitCard(radius: standard)` of metric blocks separated by hairline dividers that reflows to two rows below ~760px. *Identity* surfaces (Profile) use the app's hero vocabulary instead: `VitCardVariant.hero` gradient + `VitHeroGlow` + `VitCardRadius.large` (`ProfileAccountHero`, 2026-08) — avatar, the member's name as the page title, tier pills, fact boxes, and a progress runway at the foot. The KPI-strip idiom on a personal identity page reads as templated; pick the banner grammar that matches what the page *is*, not the one the previous page used. Either way the banner never scrolls with either column.
