...
**Reference screens:** 4 root tabs render a monitor-dashboard body through `VitTwoColumnTabletDashboard`: `home_tablet_page.dart` (first, R1-R9 originally established here); `wallet_tablet_page.dart` (confirmed R9's header-promotion pattern generalizes); `profile_tablet_page.dart` (confirmed a nested inner screen-state switch upstream of the two-column body doesn't need to interact with the shared widget at all); `markets_tablet_page.dart` (was the third dashboard reference until 2026-08-24, when Markets moved to the Analysis-terminal master-detail shell below). `trade_tablet_page.dart` was the fifth dashboard reference until 2026-08-31, when Trade moved to the fixed-grid Trading-terminal composition (archetype table above) — its financial-safety column invariant (order-entry backbone + risk panel together) survived the move unchanged, now as an always-visible terminal column. After all dashboard references confirmed the identical scaffold, it was extracted into `VitTwoColumnTabletDashboard` (own test: `test/shared/layout/vit_two_column_tablet_dashboard_test.dart`) and all remaining pages migrated onto it — see "Upgrade path" below.
...
| --- | --- | --- | --- |
| **Monitor dashboard** — watch state and act fast (Home, Wallet, Profile) | `VitTwoColumnTabletDashboard` (R4–R8) + thin banner strip + fixed header sibling (R9) | `home_tablet_page.dart`, `wallet_tablet_page.dart`, `profile_tablet_page.dart` | *What goes in the columns* (portfolio vs order form + risk panel) and the banner's metric grammar |
| **Trading terminal** — read live market and place orders now (Trade) | Fixed grid owning the full viewport height, **no page scroll** (only book/tape/tables scroll internally): dense 1-line meta strip (pair switcher + price + Cao/Thấp/KL + refresh) above [chart-candles column (product tabs + OHLC toolbar + crosshair + bottom tabs Lệnh mở\|Vị thế) \| order-book 12-level + trade-tape column \| always-visible order-entry column (form + risk panel + disclaimer, internal scroll only)]. Flat hairline panels (`VitCardRadius.tight` + micro label) separated by border + label, not gaps — the Markets SC-044 hướng C panel language. Width tiers read width ONCE via LayoutBuilder: ≥`tradeTerminalFullSplitMinWidth` (1000) 3 zones; ≥`tradeTerminalSplitMinWidth` (620) chart \| entry+tape (book becomes a third bottom tab); below → stacked scroll (window-resize only). Financial-safety invariant: the order-entry backbone (form + risk panel) stays together in its always-visible column | `trade_tablet_page.dart` (2026-08-31, hướng Bybit user duyệt) | Which panels the middle column stacks (book+tape), the chart toolbar's timeframes/indicators, and the bottom tabs' tables — the market data itself |
...
| **Analysis terminal** — browse a live list while inspecting the selection side-by-side (Markets) | Same `StatefulShellRoute` shell idiom, but the master is a *working list* (search + watchlist-first category chips + sort row + compact rows), not a settings menu (`MarketsTabletMasterShell`). The pair pane itself is width-tiered (Hướng 1 "Trading Desk", 2026-08-29): pane ≥ `pairDeskSplitMinWidth` (700dp, measured) splits into a chart main column + a fixed 300dp side column (order book + trades always visible, mini-tab Biểu đồ \| Độ sâu) with a pinned trade footer (`MarketsPaneScaffold.footer`: compact price + MUA/BÁN, never scrolls); below it the single-column 4-tab composition. The branch reads pane WIDTH once via LayoutBuilder — never orientation | `markets_tablet_master_shell.dart` | What the master list filters/sorts by, and which analysis panes the detail column renders (`/markets` overview, `/pair/...`) |
...
| **Linear detail / content page** — one thing to read or confirm (pair detail, order receipt, prediction event) | **No dedicated two-column composition** — renders in the shared nav-rail shell (file page tablet vẫn tồn tại cho từng route và phải qua registry scaffold T1 của Tablet-Composition-Tier-Standard — chỉ là KHÔNG cần dashboard/master-detail riêng); the module's tablet dashboard links into it | receipt/event pages under `presentation/tablet/pages/` | Content only; the layout is the shell's |
...
3. **Shell owns header and framing; panes own their scroll.** The shell
   ...
   (`TabletDashboardWidths` 900/800/400/12/12: outer margin + column gutter,
...
- Five dashboard reference implementations existed at the scaffold's peak (`HomeTabletPage`, `WalletTabletPage`, `MarketsTabletPage`, `TradeTabletPage`, `ProfileTabletPage`) before Markets (2026-08-24) and Trade (2026-08-31) graduated to their own terminal archetypes. R8's 900 threshold and current 400 secondary cap held unchanged for all five, across genuinely varied content shapes (a financial order-entry form, a nested inner screen-state switch, a search-filterable list); the 640 primary cap held structurally (no overflow) but was later found too conservative on a wide physical tablet — raised to 760 and then 800 (R5/R8) after on-device verification, not just a desk-check. Re-validate before assuming any of these numbers hold for a screen with meaningfully different content density, and pass constructor overrides on `VitTwoColumnTabletDashboard` rather than editing the shared defaults if they don't. Trade's terminal thresholds (`tradeTerminalFullSplitMinWidth` 1000 / `tradeTerminalSplitMinWidth` 620, `TradeSpacingTokens`) were verified by the QA-width matrix in `trade_tablet_page_test.dart`, per R3's measure-don't-guess rule.

## Upgrade path

Markets left the dashboard-reference set on 2026-08-24 for the Analysis-terminal master-detail shell, and Trade left it on 2026-08-31 for the fixed-grid Trading-terminal composition — `VitTwoColumnTabletDashboard` retains three reference consumers (Home, Wallet, Profile).
