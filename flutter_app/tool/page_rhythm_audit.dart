...
bool _headerHasInnerGap(String callBody, String source, int callStart) {
  if (RegExp(
    r'bottomGap:\s*(?:(?:AppSpacing|TabletSpacingTokens|AppSurfaceSpacing)\.pageRhythm\w+InnerGap|VitDensity\.|VitContentGap\.)',
  ).hasMatch(callBody)) {
    return true;
  }
  ...
  if (RegExp(
    r'bottomGap:\s*(?:(?:AppSpacing|TabletSpacingTokens|AppSurfaceSpacing)\.(?:x4|cardGap))',
  ).hasMatch(callBody)) {
    return true;
  }
  ...
  if (RegExp(
    r'SizedBox\s*\(\s*height:\s*(?:AppSpacing|AppSurfaceSpacing)\.(?:tradePageContentGap|pageRhythm\w+InnerGap|x[23])',
  ).hasMatch(after)) {
    return true;
  }
