...
void main() {
  ...
  testWidgets('SC-157 Tablet uses the real edit pane composition', (
    tester,
  ) async {
    // The edit sub-route migrated off the placeholder into the
    // master-detail real pane — deep-linking straight to it still lands on
    // the full editor inside the shell.
    await pumpTabletRoute(tester, AppRoutePaths.profileEdit);

    expect(find.byType(ProfileEditPane), findsOneWidget);
    expect(find.byType(ProfileTabletUtilityPage), findsNothing);
