...
/// Pane chi tiết giao dịch SC-141 (tablet): render bên trong
/// `WalletTabletHistoryShell`, không tự vẽ top chrome trừ tầng hẹp
/// (<680 — window-resize) nơi master list không còn kề pane, back header
/// riêng là đường quay lại danh sách (idiom `MarketsPaneScaffold`).
...
class _TransactionDetailTabletPageState
  ...
  Widget build(BuildContext context) {
    ...
    return snapshotAsync.when(
      loading: () => _pane(children: [const VitSkeletonList()]),
      error: (error, stackTrace) => _pane(
        children: [
          VitErrorState(
      ...
      data: (snapshot) {
        ...
        return _pane(children: _buildContent(transaction));
      },
    );
  }

  /// Content pane một cột: hero trạng thái + tiến trình + thông tin chi
  /// tiết + hành động. Pane trong split tier rộng ~900dp, nội dung thông
  /// tin đọc tốt ở một cột; dashboard hai cột không cần cho màn chi tiết.
  ...
  List<Widget> _buildContent(WalletTransaction transaction) {
